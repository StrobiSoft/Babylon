...
describe('default language agent integration', () => {
  it('passes neutral content unchanged', async () => {
    let calls = 0;
    ...
    await expect(
      agent(engine).process({
        ...request,
        sourceText: 'https://example.com/a?b=1',
      }),
    ).resolves.toEqual({
  ...
  it('bypasses same-language input', async () => {
    let calls = 0;
    const engine: ModelEngine = {
      generate: () => {
        calls += 1;
  ...
  it('delivers a valid primary translation', async () => {
  ...
  it('repairs a wrong-language primary candidate', async () => {
    const calls: ModelEngineRequest[] = [];
    const engine: ModelEngine = {
      generate: (input) => {
        calls.push({ ...input });
    ...
    const engine: ModelEngine = {
      generate: (input) => {
        ...
        if (input.modelId === 'secondary:test') {
          return Promise.resolve({
            text: 'Szia, ez a másodlagos fordítás.',
          });
        }
  ...
  it('uses reserve only after earlier recovery paths fail', async () => {
    const calls: ModelEngineRequest[] = [];
    const engine: ModelEngine = {
      generate: (input) => {
        calls.push({ ...input });
        if (input.modelId === 'reserve:test') {
          return Promise.resolve({
            text: 'Szia, ez a tartalék fordítás.',
          });
        }
        return Promise.resolve({ text: 'Hello, still wrong.' });
      },
  ...
  it('returns pending when all candidates fail validation', async () => {
    const engine: ModelEngine = {
